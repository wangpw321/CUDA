...
using namespace std;
//-------------------------------------------------------------------------------------------
static const long ARRAY_LENGTH = 2048;
...
//-------------------------------------------------------------------------------------------
__global__ void count_if(int *A_d, int* count, int val, const long aLength, int sec) {

  //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  // ****      Write Kernel Code       ****
  //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  __shared__ int A_ds[2*BLK_SIZE];

  unsigned int t = threadIdx.x;
  unsigned int start = 2 * blockIdx.x*blockDim.x;
  A_ds[t]=(A_d[sec*2*BLK_SIZE+start+t]!=val) ? 0 : 1;
  A_ds[blockDim.x+t]=(A_d[sec*2*BLK_SIZE+blockDim.x+start+t]!=val) ? 0 : 1;
...
int main(void) {
  ...
  for(int sec=0; sec<(ARRAY_LENGTH/(2*BLK_SIZE)); sec++){
    count_if<<<1, BLK_SIZE>>>(A_d, count_d, val, ARRAY_LENGTH, sec);
  }
